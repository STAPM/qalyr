...
#'
#' @param inpatient_data Data table - inpatient data containing data on the patient's
#' admission and discharge dates, and diagnosis codes.
#' @param survey_data Data table - the HODaR survey data containing EQ5D
#' @param lkup Data table - lookup table in the format (icd_code, Description)
#' (default stored in package as qalyr::lkup - this contains all
#' alcohol and tobacco related conditions).
#'
#' @return Returns a table with the survey data and inpatient data merged,
...
merge_data <- function(
  ...
  # Loop through each diagnosis column and search for ICD-10 codes that
  # match alcohol/tobacco-related conditions.
  ...
  causes <- c("d4_cause_1", "d3_cause_1", "d4_cause_2", "d3_cause_2", "d4_cause_3",
              ...
              "d3_cause_8", "d4_cause_9", "d3_cause_9", "d4_cause_10",
              "d3_cause_10", "d4_cause_11", "d3_cause_11", "d4_cause_12", "d3_cause_12",
              "d4_cause_13", "d3_cause_13")
  ...
  survey_data <- survey_data[ , list(Patient_Number, Date_Survey_Sent,
                                          Date_Survey_Returned, Age, gender, Smoke,
                                          Alcohol, Utility_value)]

  inpatient_data <- merge(inpatient_data, survey_data,
                          by = "Patient_Number", all = T, allow.cartesian = TRUE)

  # Need to create an algorithm that chooses only the rows that make sense
  # (i.e. PS_Disch < Date_Survey_Sent, but not years before).
