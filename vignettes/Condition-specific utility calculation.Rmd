...
```{r install package, eval = T, echo = F, warning = F, message = F, cache = FALSE}
...
```{r util, eval = T, warning = F, echo = F, message = F, cache = FALSE}
library(knitr)
...
```{r gen pop, eval = T, echo = F, warning = F, message = F, cache = F}
...
```{r age male proportion, eval = T, warning = F, message = F, cache = F}
...
```{r pneumonia, eval = T, echo = F, warning = F, out.extra = '', fig.width = 8, fig.height = 6, fig.cap = "Comparison of age-specific HSUVs for the general population and Pneumonia."}
