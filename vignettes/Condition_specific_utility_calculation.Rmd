...
## HODaR data
The `qalyr` package contains functions to read and process the HODAR data [@currie2005routine] into a suitable form for use in our modelling. The raw data is in two files: the inpatient data `read_inpatient_data()` and the survey data `read_survey_data()`. The raw data was provided to us with only episodes with at least one tobacco- or alcohol-related diagnosis code in it. 
...
The multiplier for a condition is obtained using the HSUV for the condition and the baseline HSUV for individuals with the same mean age and proportion of males as the condition `MeanAge_Sex()`.   
